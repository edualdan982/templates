...
public interface IUsuarioRolDao extends JpaRepository<UsuarioRol, IdUsuarioRol> {
  ...
  @Query(value = "SELECT ur FROM UsuarioRol ur "
      + "WHERE ur.eliminado = false AND ur.id.idUsuario = :idUsuario AND ur.fechaVencimiento >= current_timestamp ORDER BY ur.id.idRol ASC")
  public List<UsuarioRol> buscarPorIdUsuario(Integer idUsuario);

  @Query(value = "SELECT ur FROM UsuarioRol ur "
      + "WHERE ur.eliminado = false AND ur.id.idUsuario = :idUsuario ORDER BY ur.id.idRol ASC")
