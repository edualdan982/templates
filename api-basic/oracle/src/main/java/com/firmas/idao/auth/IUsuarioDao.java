...
public interface IUsuarioDao extends JpaRepository<Usuario, Integer> {
  ...
  @Query(value = "SELECT u FROM Usuario u WHERE u.username = :username AND u.eliminado = false")
  Usuario findByUsuario(String username);

  @Query("select u from Usuario u where u.username=?1 and u.estado = true AND u.eliminado = false")
  ...
  @Query("SELECT u FROM Usuario u JOIN FETCH u.roles r WHERE u.username = :username AND (r.eliminado = false AND r.id.idUsuario = u.idUsuario)")
  Optional<Usuario> buscarPorUsuarioFetchRole(String username);

  @Query(value = "SELECT u FROM Usuario u WHERE u.eliminado = false ORDER BY u.idUsuario DESC")
  List<Usuario> listarUsuarios();

  @Query(value = " SELECT COALESCE(COUNT(*),0) FROM Usuario WHERE username = :username")
  Integer contarUsuario(String username);

  @Modifying
  @Query(value = "UPDATE Usuario SET eliminado = true,username=CONCAT(username,'-',idUsuario), fechaModificacion = current_timestamp WHERE idUsuario = :idUsuario")
  void eliminarPorId(Integer idUsuario);

  @Modifying
  @Query(value = "UPDATE Usuario SET estado = false, fechaModificacion = current_timestamp WHERE username = :username")
  ...
  @Query(value = "SELECT u FROM Usuario u WHERE u.eliminado = false AND u.claveWeb =:claveWeb")
  ...
  @Query(value = "SELECT u FROM Usuario u WHERE u.eliminado = false AND " +
  ...
  @Modifying
  @Query("UPDATE Usuario u SET u.password = :clave WHERE u.idUsuario = :idUsuario AND u.eliminado = false")
  void cambiarClave(Integer idUsuario, String clave);

  @Query("SELECT u.idUsuario FROM Usuario u WHERE u.username = :username AND u.eliminado = false")
